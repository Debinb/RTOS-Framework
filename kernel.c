...
void svCallIsr(void)
{
    ...
    case 2:
    {
        Taskticks = *getPSP();                      //grabs the value of the ticks from PSP address
    ...
    case 13:
    {
        uint32_t TaskPID = *getPSP();                       // Get the PID if Kill() is called
        char *TaskName = (char*)*getPSP();                  // Get the Name if Pkill() is called

        uint8_t task = 0;
        for(task = 0; task < MAX_TASKS; task++)             // Go through the TCB
        {
                                                            // If the thread's PID or name is in the TCB
            if((TaskPID == (uint32_t)tcb[task].pid) || ((cmpStr(tcb[task].name, TaskName)) == 0))
            {
                ...
                {
                    //Implement Unlock logic here for shifting queue forward
                    uint8_t MutexIndex = tcb[task].mutex;   // Use the mutex index from tcb. Will be 0
                    uint8_t i,j = 0;
                    for (i = 0; i < mutexes[MutexIndex].queueSize; i++) // Go through the process queue and check for the task
                    {
                        if (mutexes[MutexIndex].processQueue[i] == task)
                        {
                            for (j = i; j < mutexes[MutexIndex].queueSize - 1; j++) // Shift remaining tasks in the queue
                            {
                                mutexes[MutexIndex].processQueue[j] = mutexes[MutexIndex].processQueue[j + 1];
                            }
                ...
                {
                    uint8_t SemaIndex = tcb[task].semaphore;
                    uint8_t i,j = 0;
                    for (i = 0; i < semaphores[SemaIndex].queueSize; i++)
                    {
                        if (semaphores[SemaIndex].processQueue[i] == task)  // Go through the semaphores process queue and remove the task
                        {
                            for (j = i; j < semaphores[SemaIndex].queueSize - 1; j++)
                            {
                                semaphores[SemaIndex].processQueue[j] = semaphores[SemaIndex].processQueue[j + 1];
                            }
                            semaphores[SemaIndex].queueSize--;
                            break;
                        }
                    }
                }
                ...
                if(mutexes[tcb[task].mutex].lockedBy == task)       // Also check if the task is locking a resource
                {
                    ...
                    mutexes[tcb[task].mutex].lockedBy = 0xFF;       // Remove the task from blocking other tasks
                }
    ...
    case 14:
    {
        ...
        for(task = 0; task < MAX_TASKS; task++)             // Go through the TCB
        {
            if((cmpStr(tcb[task].name, TaskName) == 0))
            {
                // allocate the stack
                // PID to PC
                // Size to SP
